...
pub fn part_one(input: &str) -> Option<u32> {
    ...
    for i in measurement_points{
        println!("{}     {} : {}", i as i32 * cpu.register_history[i - 1], i, cpu.register_history[i -1]);
    }
    let measures = measurement_points.map(|p| p as i32 * cpu.register_history[p - 1])
                                          .iter().sum::<i32>();
...
impl Processor{
    ...
    fn noop(&mut self){
        println!("{} clock [{:0>3}] Register      {: >3}", format!("NOOP ").blue(), self.clock, self.X);
        
        self.clock += 1;
        self.register_history.push(self.X);
    }
    fn add_x(&mut self, value: i32){
        println!("{} clock [{:0>3}] Register(pre) {:0>3} value: {: >4} ", format!("ADDX=").red(), self.clock, self.X, value);
        self.clock += 1;
        self.register_history.push(self.X);

        println!("{} clock [{:0>3}] Register(pre) {: >3} value: {: >4} ", format!("ADDX+").red(), self.clock, self.X, value);
        self.clock += 1;
        self.X += value;
        self.register_history.push(self.X);
    }
