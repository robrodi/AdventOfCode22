            ...
            for i in 0..rope_length{
                ...
                if (last.x - tail.x).abs() > 1 || (last.y - tail.y).abs() > 1{
                    if last.x != tail.x && last.y != tail.y { // move diagonally
                        if last.x > tail.x { tail.x += 1; }
                        if last.x < tail.x { tail.x -= 1; }
                        if last.y > tail.y { tail.y += 1; }
                        if last.y < tail.y { tail.y -= 1; }
                    }
                    else{
                        if last.x - tail.x > 1 { tail.x += 1; }
                        if tail.x - last.x > 1 { tail.x -= 1; }
                        if last.y - tail.y > 1 { tail.y += 1; }
                        if tail.y - last.y > 1 { tail.y -= 1; }
                    }
                }
                current = *tail;
            }
